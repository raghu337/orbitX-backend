...
class ExtendedReporter:
    ...
    def suggest_fix(test_name, exception_str):
        ...
        elif "unauthorized" in name_lower:
            return "Verify that sessionStorage/localStorage clear actions are executing correctly in conftest.py."
    ...
    def generate_reports(results, env_details, reports_dir):
        ...
        ExtendedReporter._write_excel(results, env_details, excel_path)
        
        # 2. Generate Markdown summaries
        ExtendedReporter._write_markdown_summary(results, env_details, summary_path)
        ExtendedReporter._write_markdown_exec_summary(results, env_details, exec_summary_path)
        ExtendedReporter._write_markdown_failed(results, failed_tests_path)
        ...
        for ws in wb.worksheets:
            for col_idx in range(1, ws.max_column + 1):
                ...
                for row_idx in range(3, ws.max_row + 1):
                    # For test results, skip column 8 width check if it contains embedded image
                    if ws.title == "Test Results" and col_idx == 8:
                        max_len = 16
                        continue
                    cell = ws.cell(row=row_idx, column=col_idx)
                    val_str = str(cell.value or '')
                    # Cap width contribution from stack trace cells to avoid giant columns
                    if len(val_str) > 100:
                        val_str = val_str[:100]
        ...
        if failed_count == 0:
            ...
            for r in results:
                ...
                if r.get("screenshot_path"):
                    # Use absolute path for embedding in markdown
